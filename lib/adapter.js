...
module.exports = (function() {
  ...
  var adapter = {
    ...
    find: function(collectionName, options, cb) {
      ...
      spawnConnection('asd', dbs[modelName].config, cb);
      modelInstance.find(options.where, function(err, records) {
        console.log('****************************************** in teh finders **********************************************************');
        console.log('%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%');
        if(err) throw err;
        console.log('***************************** records **************************************');
        console.log(records);
  ...
  function spawnConnection(logic, config, cb) {
    ...
    var mon = mongoose,
        conInfo = 'mongodb://localhost/sails';

    /**
      * ref: http://mongoosejs.com/docs/api.html#connection_Connection-readyState
      */
    mon.connect( conInfo, function(err) {
      if (err) {
        switch(err.state){
          case 0: 
            console.log('The mongo server has disconnected');
            break;
          case 1: 
            console.log('The mongo server is connected');
            moveon();
            break;
          case 2: 
            console.log('The mongo server is connecting');
            break;
          case 3: 
            console.log('The mongo server is disconnecting');
            break;
        };
      };
    });
